...
export class EstadisticasService extends EstadisticasRepository {
  ...
  getEstadisticas(usuarioId: number): Observable<EstadisticasUsuario> {
    return this.http$.get<EstadisticasUsuario>(
      `${EstadisticasAPI.PorUsuario}${usuarioId}`
    );
  }
  ...
  mapearResumenEstadisticas(data: EstadisticasUsuario): ResumenEstadisticas {
    return {
      misRecursos: {
        valor: data.misRecursos,
        titulo: 'Mis Recursos',
        icon: 'fas fa-box',
      },
      intercambios: {
        valor: data.intercambios,
        titulo: 'Intercambios',
        icon: 'fas fa-exchange-alt',
      },
      intercambiosCompletados: {
        valor: data.intercambiosCompletados,
        titulo: 'Intercambios Completados',
        icon: 'fas fa-check-circle',
      },
      cancelaciones: {
        valor: data.cancelaciones,
        titulo: 'Cancelaciones',
        icon: 'fas fa-times-circle',
      },
      usuariosContactados: {
        valor: data.usuariosContactados,
        titulo: 'Usuarios Contactados',
        icon: 'fas fa-users',
      },
      articulosPublicadosMes: {
        valor: data.articulosPublicadosMes,
        titulo: 'Artículos este Mes',
